...
/*
	...
	// contador para parar o programa automaticamente
	int cont = 0;
	
	// parar com o comando do teclado	
	//while(kb_kbhit()==0)
	// parar apos 200 segundos
	while(cont<200)
	{
	  kb_clrscr();	
		...
		cont+=1;
		// para parar com o comando do teclado deve-se descomentar a linha abaixo
		//printf("\n\nPush any key to continue program\n");
...
/*
	...
	// iniciando um novo contador para o teste abaixo
	int contador = 0;
	
	// aguarda 5s 
	sleep(5);
	
	// linha abaixo é original do teste, caso queira utilizar descomente a mesma
  	// while(kb_kbhit()==0)
  	while (contador<200)
	{
		kb_clrscr();	
...
/**/
	...
	{	
		...
		printf("\nUS sensors : distance [cm]\
		        \n                     50  100  150  200\
		        \n                0|    .    :    .    :    |%.0f\nleft 90   : %4d %s  %s\nleft 45   : %4d %s  %s\
		\nfront     : %4d %s  %s\nright 45  : %4d %s  %s\nright 90  : %4d %s  %s\n", MAX_US_DISTANCE, usvalues[0], bar[0], US_VAL(usvalues[0]), usvalues[1], bar[1], US_VAL(usvalues[1]), usvalues[2], bar[2], US_VAL(usvalues[2]), usvalues[3], bar[3], US_VAL(usvalues[3]), usvalues[4], bar[4],US_VAL(usvalues[4])); 
