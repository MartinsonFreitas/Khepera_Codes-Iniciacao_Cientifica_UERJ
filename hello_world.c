...
/*
// Função para tratar o sinal de interrupção via teclado
static void ctrlc_handler( int sig ) 
{
  quitReq = 1;
  
  kh4_set_speed(0 ,0 ,dsPic); // stop robot
  kh4_SetMode( kh4RegIdle,dsPic ); // set motors in idle mode  
...
{ 
  ...
  double fpos,dval; // position in mm
  int lpos,rpos; // position in pulse
  char Buffer[100],bar[12][64],revision,version; // misc var
  int i,n,val,type_of_test=0,sl,sr,pl,pr,mean; // misc var
  short index, value,sensors[12],usvalues[5]; // misc var
  char c; // misc var
  long motspeed; // speed in pulse/10ms
  char line[80],l[9]; // misc var
  int kp,ki,kd; // SetPID
  int pmarg,maxsp,accinc,accdiv,minspacc, minspdec; // SetSpeedProfile  
  
  //printf("\nKhepera 4 small example program\n");
  printf("\nHello World!\n");
  printf("\nRetire o Cabo USB, por favor!\n");
  ...
  /* tuned parameters */
  pmarg=20; // position control margin
  ...
  accinc=3; // Acceleration increment
  accdiv=0; // Acceleration divider
  minspacc=20; // Minimum speed acc
  minspdec=1; // Minimum speed dec
  maxsp=400; // Maximum speed
  ...
  //kh4_SetMode( kh4RegSpeedProfile,dsPic ); // Put in speed profile mode
  //kh4_SetMode( kh4RegPosition,dsPic ); // Put in position mode

  // get revision
  //printf("\nKhepera IV revision: %c\n",kh4_revision(Buffer, dsPic));
  ...
  //printf("\nKhepera IV serial number: %s\n",kh4_serial(Buffer, dsPic));
  //signal( SIGINT , ctrlc_handler ); // set signal for catching ctrl-c
  // set keyboard in raw mode
...
//printf("\nRGB leds : front...\n");
int cont; // variável cont não está declarada no início do programa
// acender os leds da frente para trás
// duas vezes
...
// -----------  rgb led example -------------------
// acender os leds de trás para frente
for (cont=0; cont<2; cont++){
//printf("\nRGB leds : back...\n");
	for (i=0;i<0x20;i++)
	{
	...
	/////////////////////////////////////////////////
	// efetuando um movimento para frente e para trás
	
	// Tell to the motor controller to move the Khepera 4 forward, in speed profile control
	...
	// robot move forward 100cm in position control
	// modificado de 10*10 para 100*20
	fpos = 100.0*20.0/KH4_PULSE_TO_MM; // compute number of pulse from mm
	...
	// Fim do bloco de movimento para frente
	//////////////////////////////////////////

/*
	//////////////////////////////////
	// Iniciando o movimento para trás
	...
	// robot move backward 100cm in speed profile control
	// modificado de -25 para -250
	motspeed= (long)(-250.0/KH4_SPEED_TO_MM_S); // convert speed from mm/s to pulse/10ms
	...
	// Fim do bloco de movimento para para trás
	///////////////////////////////////////
*/
	///////////////////////////////////////
	// inicializando o movimento de rotação

	// Tell to the motor controller to stop the Khepera 4
	kh4_set_speed(0 ,0,dsPic);
	sleep(1);	 // Wait 1 seconds

	// Pega a posição dos encoders
	kh4_get_position(&lpos,&rpos,dsPic);
	//printf("\n encoders: left %ld | right %ld\n",lpos,rpos);
	
	// Prepare to rotate 180° // modificado de 40 para 33
	motspeed= (long)(33.0/KH4_SPEED_TO_MM_S); // convert speed from mm/s to pulse/10ms
	// Tell to the motor controller to rotate the Khepera 4, in speed control
	...
	sleep(5); // Wait 5 seconds
	kh4_set_speed(0 ,0 ,dsPic); // stop robot
	// Fim do bloco de movimento de rotação
	////////////////////////////////////////
...
/*
	...
	//printf("\nPosition factor: pulse to mm: %lf\nSpeed factor: speed to mm/s: %lf\n",KH4_PULSE_TO_MM,KH4_SPEED_TO_MM_S);
	// robot move forward 100cm in position control
	// modificado de 100 para 10
	fpos = 100.0*10.0/KH4_PULSE_TO_MM; // compute number of pulse from mm
	//printf("\nMoving forward %.1f cm in  %.1f pulses with position control\nencoders: left %ld  (final expected %ld) | right %ld (final expected %ld)\n",10.0,fpos,lpos,lpos+(long)fpos,rpos,rpos+(long)fpos);
...
/*
	//////////////////////////////////////////////
	// inicializando o movimento de rotação em loop
	// Pega a posição dos encoders
	kh4_get_position(&lpos,&rpos,dsPic);
	//printf("\nread encoders: left %ld | right %ld\n",lpos,rpos);
	
	// Open loop
	motspeed= KH4_MAX_OPENLOOP_SPEED_PWM_100/2;  // 25% // modificado de 4 para 2
